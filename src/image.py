...
class Image:
    ...
    def save(self, file_path: str = None) -> str:
        ...
        if self.convert_ybr_to_rgb and self.image_dataset.PhotometricInterpretation in ['YBR_FULL', 'YBR_FULL_422']:
            pixel_array = \
                convert_color_space(
                    arr=pixel_array, current=self.image_dataset.PhotometricInterpretation, desired="RGB")
